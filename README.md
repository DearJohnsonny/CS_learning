...
<div align=center>
<img src="https://github.com/DearJohnsonny/CS_learning/assets/111955215/8a355180-48bc-492e-92f3-9e6d205cae0e" width="400">
</div>
...
ctrl+c: 杀死当前进程（开启新的命令行）
ctrl+d: 退出当前 命令行窗口 Shell（注意：在命令行没有输入任何字符时才有效）
 
ctrl+w: 清除光标前的一个单词，
比如：mkdir abcde，若光标在c上，则移除ab；若光标在e上，则移除abcd；若光标在a上，则移除mkdir
...
回车直接执行查询的命令；→方向键在命令行显示该命令
 
ctrl+t: 把光标位置和光标前一个字符进行交换，然后光标会向后退一格
 
ctrl+z: 把当前进程转到后台运行，使用fg命令恢复。比如top ，然后ctrl+z ，到后台，然后fg，重新恢复
